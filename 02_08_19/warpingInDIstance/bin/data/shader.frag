...
float fbm( in vec2 x )
{
    ...
    float f =  -1.0;
    float a = 1.0;//(cos(u_time * TWO_PI * 0.05)) * 0.25 + 0.8;//noise(vec2(u_time*0.5  , x.y)* 0.25) + 0.8 ;//1.1;1.0;
...
//         if(texture(tex,  varyingtexcoord).r >= 0.5 && texture(tex,  varyingtexcoord).g <= 0.8 &&texture(tex,  varyingtexcoord).b <= 0.8 ){
     ...
     inputImgDist = texture(tex,  varyingtexcoord - pat * u_resolution * (1.0 - dist(mouseNorm   ,st   )*2.0) );
    //inputImgDist = texture(tex,  varyingtexcoord *pat);// (    ( pat*u_resolution )   * mouseNorm.x ) );
...
//    if(st.x < 0.25)
//        st.x = 0.0;
//    if(inputImg == vec4(vec3(1.0), 1.0)){
////    inputImg = vec4(vec3(0.4*pat * st.x, 0.3* pat * st.y, pat), 1.0);
//        inputImg =  vec4(vec3(pat , 0.1* pat , pat*0.1 ), 1.0);
////        inputImg =  vec4(vec3(st.y * 0.2 + 0.1 ), 1.0);
//
//    }
    
    
    if(inputImgDist == vec4(vec3(0.0), 1.0)){
        inputImgDist =  inputImg;//vec4(vec3(0.4*pat, 0.3* pat , pat * 0.7), 1.0);//inputImg;//vec4(vec3(pattern(st)), 1.0);
// inputImgDist =  vec4(vec3(st.y * 0.5), 1.0);
    }else{
//        inputImgDist.x*=pat;
//        inputImgDist*=(pat *0.1);
