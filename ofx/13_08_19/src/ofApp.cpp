...
//    plane.set(ofGetWidth(), ofGetHeight(), ofGetWidth(), ofGetHeight(), OF_PRIMITIVE_TRIANGLE_STRIP);
    ...
    ofTrueTypeFont f;
    f.loadFont("/Users/edmundoetgen/Documents/of_v0.10.1_osx_release/apps/myApps/type1/bin/data/HelveticaNeue-01.ttf", 200);
...
//        rangerange = 50.;
   ...
   if(ofGetElapsedTimeMillis() > unlockTimer)
    ...
    std::cout <<burst<< " " << burstA <<std::endl;
}
...
//    shader.setUniform1f("mouseX", mousePosition);
    shader.setUniform1f("mouseX", sin(ofGetElapsedTimef()) * range);
    shader.setUniform1f("mouseY", cos(ofGetElapsedTimef()*1.4) * range *0.8);
