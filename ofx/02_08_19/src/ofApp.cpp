...
void ofApp::draw(){
    ...
    float pulse =abs(sin(ofGetElapsedTimef()*2))* 100;
    std::cout<< pulse<<std::endl;
    if (pulse <= 3
        ){
        
        pos = ofVec2f(ofRandom(-ofGetWidth()/2, ofGetWidth()/2), ofRandom(-ofGetHeight()/2, ofGetHeight()/2));
    }
    shader.begin();
    ...
    shader.setUniform1f("mouseRange", pulse);
    ...
    shader.setUniform2f("mousePos", pos.x, pos.y);
